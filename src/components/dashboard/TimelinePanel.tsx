      ...
      <div className="flex flex-col gap-4">
        <div className="flex justify-between items-center border-b border-[var(--border-subtle)] pb-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold font-display tracking-wider uppercase text-[var(--text-primary)]">WORKSPACE SYNC STREAM</span>
          </div>
        ...
        {activities.length === 0 ? (
          ...
          <div className="relative pl-4 border-l border-[var(--border-subtle)] space-y-5">
            {activities.map(log => {
              ...
              return (
                ...
                >
                  <div className={`absolute -left-[25px] top-[1px] w-4 h-4 rounded-full border flex items-center justify-center transition-all group-hover:scale-110 ${color}`}>
                    <Icon className="w-2.5 h-2.5" />
                  </div>
                  <div className="flex flex-col gap-1">
                    <div className="flex items-center justify-between">
                      <span className="text-[9px] font-mono font-extrabold tracking-wider text-[var(--text-muted)]">{label}</span>
                      <span className="text-[9px] font-mono text-[var(--text-muted)] flex items-center gap-1">
                        <Clock className="w-2.5 h-2.5" />
                        <ClientDate iso={log.timestamp} type="time" />
                      </span>
                    </div>
                    <h4 className="text-xs font-medium text-[var(--text-secondary)] group-hover:text-cyber-cyan transition-colors truncate pr-4">{log.noteTitle}</h4>
                  </div>
                </div>
              );
            })}
